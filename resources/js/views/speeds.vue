    ...
    <q-page class="q-ma-md" padding>
        ...
        <q-card>
            <q-card-section class="row no-padding">
                <q-toolbar class="bg-primary text-white rounded-borders">
                    <q-icon name="mdi-speedometer" size="md"/>
                    <q-toolbar-title>
        ...
        <q-dialog ref="dlg">
            <q-card class="q-dialog-plugin">
                ...
                <q-card-section>
                    <q-input v-model="$v.speed.description.$model" label="Descripción"
                    :error="$v.speed.description.$invalid"/>
                </q-card-section>
...
<script>
    ...
    export default {
        ...
        computed: {
            isManageable() {
                return _.indexOf(this.$store.state.main.grants, grants.gSpeeds) !== -1;
            },
            caption() {
        ...
        methods: {
            async onRequest(props) {
                ...
                await this.$axios.post('/api/speeds/list', {
                    startRow, rowsPerPage, filter, sortBy, descending
                }).then(r => {
                    if (r.data.code !== 'Ok') return Error.call(this, resources.eGetSpeeds);
                    this.speeds = r.data.dt.rows;
                    ...
                    Error.call(this, resources.eGetSpeeds);
                    console.error(e);
                });
            },
            create () {
            ...
            remove(id) {
                AskForRemove.call(this).onOk(_ => {
                    this.$axios.post('/api/speed/destroy', {id}).then(r => {
                        if (r.data.code !== 'Ok') return Error.call(this, resources.eDestroySpeed);
                        this.onRequest({pagination: this.pagination, filter: this.filter});
                        Success.call(this, resources.sDestroySpeed);
                    }).catch(e => {
                        Error.call(this, resources.eDestroySpeed);
                        console.log(e);
                    });
                });
            },
            save () {
                let urlAction = '/api/speed/' + this.action,
                    msg = this.action === 'store' ? [resources.eStoreSpeed, resources.sStoreSpeed] : [resources.eUpdateSpeed, resources.sUpdateSpeed];
                this.$axios.post(urlAction, {...this.speed}).then(r => {
