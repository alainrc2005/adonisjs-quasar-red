    ...
    <q-page class="q-ma-md" padding>
        ...
        <q-card>
            <q-card-section class="row no-padding">
                <q-toolbar class="bg-primary text-white rounded-borders">
                    <q-icon name="mdi-notebook" size="md"/>
                    <q-toolbar-title>
                        Gestión de Recursos
        ...
        <q-dialog ref="dlg">
            <q-card class="q-dialog-plugin">
                ...
                <q-card-section>
                    <q-input v-model="$v.resource.description.$model" label="Descripción"
                             :error="$v.resource.description.$invalid"/>
                </q-card-section>
...
<script>
    ...
    export default {
        ...
        computed: {
            isManageable() {
                return _.indexOf(this.$store.state.main.grants, grants.gResources) !== -1;
            },
            caption() {
        ...
        methods: {
            async onRequest(props) {
                ...
                await this.$axios.post('/api/resources/list', {
                    startRow, rowsPerPage, filter, sortBy, descending
                }).then(r => {
                    if (r.data.code !== 'Ok') return Error.call(this, resources.eGetResources);
                    this.resources = r.data.dt.rows;
                    ...
                    Error.call(this, resources.eGetResources);
                    console.error(e);
                });
            },
            create () {
            ...
            remove(id) {
                AskForRemove.call(this).onOk(_ => {
                    this.$axios.post('/api/resource/destroy', {id}).then(r => {
                        if (r.data.code !== 'Ok') return Error.call(this, resources.eDestroyResource);
                        this.onRequest({pagination: this.pagination, filter: this.filter});
                        Success.call(this, resources.sDestroyResource);
                    }).catch(e => {
                        Error.call(this, resources.eDestroyResource);
                        console.log(e);
                    });
                });
            },
            save () {
                let urlAction = '/api/resource/' + this.action,
                    msg = this.action === 'store' ? [resources.eStoreResource, resources.sStoreResource] : [resources.eUpdateResource, resources.sUpdateResource];
                this.$axios.post(urlAction, {...this.resource}).then(r => {
